...
//
// n-Body Kernel for the speed calculation on device 0
//
__global__ void
simpleNbody_Kernel_device0(int numElements, float4* bodyPos, float3* bodySpeed)
{
	...
	// Update first half elements on device 0
	if (elementId < numElements/2) {
...
//
__global__ void
simpleNbody_Kernel_device1(int numElements, float4* bodyPos, float3* bodySpeed)
{
	int elementId = numElements/2 + blockIdx.x * blockDim.x + threadIdx.x;

	float4 elementPosMass;
	float3 elementForce;
	float3 elementSpeed;

	// Update second half elements on device 1
...
//
__global__ void
updatePosition_Kernel_device0(int numElements, float4* bodyPos, float3* bodySpeed)
{
	int elementId = blockIdx.x * blockDim.x + threadIdx.x;
...
//
__global__ void
updatePosition_Kernel_device1(int numElements, float4* bodyPos, float3* bodySpeed)
{
	int elementId = numElements/2 + blockIdx.x * blockDim.x + threadIdx.x;
	...
	// Update second half elements on device 0
	if (elementId < numElements) {
		elementPosMass = bodyPos[elementId];
		elementSpeed = bodySpeed[elementId];
...
//	srand(static_cast<unsigned>(time(0)));
    ...
    // Peer Access for Device 1 on Device 0
	cudaSetDevice ( 1 );
	...
	for (int i = 0; i < numIterations; i ++) {
		...
		simpleNbody_Kernel_device0<<<grid_dim, block_dim>>>(numElements, d_particles.posMass,
				d_particles.velocity);
		cudaSetDevice ( 1 );
		simpleNbody_Kernel_device1<<<grid_dim, block_dim>>>(numElements, d_particles.posMass,
				d_particles.velocity);

		cudaDeviceSynchronize();

		cudaSetDevice ( 0 );
		updatePosition_Kernel_device0<<<grid_dim, block_dim>>>(numElements, d_particles.posMass,
				d_particles.velocity);
		cudaSetDevice ( 1 );
		updatePosition_Kernel_device1<<<grid_dim, block_dim>>>(numElements, d_particles.posMass,
				d_particles.velocity);
		//cudaDeviceSynchronize();
