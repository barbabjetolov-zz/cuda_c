...
{
	// this function return the squaredistance between a and b 
...
{
	// this function evaluate the partial interaction the GAMMA factor
	// and the body A weight are a common factor nd 
	float temp = getDistance(bodyA, bodyB);
	temp = bodyB.w/__powf(temp + EPS, 1.5);
...
float Energy_Kernel(int numElements, float4* bodyPos, float3* bodySpeed)
{
	...
	for (int j = 0; j < numElements; j++){
		...
		for (int i = 0; i < numElements; i++) {
				...
				Energy += (- GAMMA * bodyPos[j].w * bodyPos[i].w)/((distance + EPS)*(distance + EPS));
		}
	}
	return Energy;
}
...
sharedNbody_Kernel(int numElements, float4* bodyPos, float3* bodySpeed)
{
	...
	extern __shared__ float4 sh_block[];
	int elementId = blockIdx.x * blockDim.x + threadIdx.x;
	int tilId     = threadIdx.x;
	float4 elementPosMass;
	float3 elementSpeed;
	...
	if (elementId < numElements){
		for (int i = elementId; i < numElements; i += blockDim.x * gridDim.x)
		{
			...
			elementPosMass = bodyPos[i];
			elementSpeed = bodySpeed[elementId];
			//#pragma unroll 16
			for (int j = 0; j < numElements; j += blockDim.x)
			{
				sh_block[tilId] = bodyPos[j + tilId];
				__syncthreads();
				//#pragma unroll 32
				for (size_t k = 0; k < blockDim.x; k++)
				{
					float3 elementForce;
					bodyBodyInteraction(elementPosMass, sh_block[k], elementForce);
					acc.x += elementForce.x;
					acc.y += elementForce.y;
					acc.z += elementForce.z;
				}
				__syncthreads();
			}

			calculateSpeed(elementPosMass.w, elementSpeed, acc);
			bodySpeed[elementId] = elementSpeed;
		}
...
//	srand(static_cast<unsigned>(time(0)));
	...
	// check energy - very slow on CPU
	bool check_energy = chCommandLineGetBool("c", argc, argv);
	// no-print for intermediate itratation 
	...
	// Shared memory size
	int sh_Size = blockSize * sizeof(float4);

	kernelTimer.start();
	1e-8*static_cast<float>(rand()); // M
	for (int i = 0; i < numIterations; i ++) {
		sharedNbody_Kernel<<<grid_dim, block_dim, sh_Size>>>(numElements, d_particles.posMass,
